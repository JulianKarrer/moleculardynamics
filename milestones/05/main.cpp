...
/// run simulation for given relaxation time and push temperature at each timestep into 
/// the given temperatures vector
...
int main(int argc, char *argv[]) {
    ...
    for(size_t factor{10}; factor<100; factor+=10){
        ...
        for(size_t current_time{0}; current_time < temps.size(); current_time++){
            file<<relaxation<<","<< (double)(current_time+1)*DT<<","<<temps[current_time]<<std::endl;
        }
    }
